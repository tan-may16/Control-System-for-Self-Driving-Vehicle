...
class EKF_SLAM():
    ...
    def _f(self, x, u):
        ...
        x_next[0]= x[0] + self.dt*(u[0]*np.cos(wrapToPi(u[2])) - u[1]*np.sin(wrapToPi(u[2])))
        x_next[1]= x[1] + self.dt*(u[0]*np.sin(wrapToPi(u[2])) + u[1]*np.cos(wrapToPi(u[2])))
    ...
    def _h(self, x):
        ...
        for i in range(self.n):
            y[i] = np.sqrt(np.square(x[3 + 2*i]-x[0] )+np.square(x[4+2*i] - x[1]))
        for i in range(self.n):
            y[self.n+i]= wrapToPi(np.arctan2(x[4+2*i] - x[1],x[3 + 2*i]-x[0])-x[2]) 
    ...
    def _compute_F(self, x, u):
        ...
        F[0,2]= -self.dt*(u[0]*np.sin(wrapToPi(u[2]))+u[1]*np.cos(wrapToPi(u[2])))
        F[1,2]=self.dt*(u[0]*np.cos(wrapToPi(u[2]))-u[1]*np.sin(wrapToPi(u[2])))
    ...
    def _compute_H(self, x):
        ...
        for i in range(self.n):
            H[i,0]= ((x[0]-x[3 + 2*i])/(np.sqrt(np.square(x[3 + 2*i]-x[0] )+np.square(x[4+2*i] - x[1]))))
            H[self.n+i,0]= ((x[4+2*i] - x[1])/(np.square(x[3 + 2*i]-x[0] )+np.square(x[4+2*i] - x[1])))
            H[i,1] = ((x[1]-x[4 + 2*i])/(np.sqrt(np.square(x[3 + 2*i]-x[0] )+np.square(x[4+2*i] - x[1]))))
            H[self.n+i,1]=((x[0]-x[3+2*i])/(np.square(x[3 + 2*i]-x[0] )+np.square(x[4+2*i] - x[1])))
            H[self.n+i,2]=-1
            H[i,3+i]= ((x[3 + 2*i]-x[0])/(np.sqrt(np.square(x[3 + 2*i]-x[0] )+np.square(x[4+2*i] - x[1]))))
            H[i,4+i] = ((x[4 + 2*i]-x[1])/(np.sqrt(np.square(x[3 + 2*i]-x[0] )+np.square(x[4+2*i] - x[1]))))

            H[self.n+i,3+i]= ((x[1]-x[4+2*i])/(np.square(x[3 + 2*i]-x[0] )+np.square(x[4+2*i] - x[1])))
            H[self.n+i,4+i]= (x[3+2*i]-(x[0])/(np.square(x[3 + 2*i]-x[0] )+np.square(x[4+2*i] - x[1])))
